...
void MainWindow::navBtnCalcPath_pressed()
{
    ...
    qtRosNode->call_static_map(map);
    bool success;
    switch(ui->navCmbMethod->currentIndex())
    {
    case 0:	
        success = qtRosNode->call_breadth_first_search(startX, startY, goalX, goalY, map, path);
        break;
    case 1:
        success = qtRosNode->call_depth_first_search(startX, startY, goalX, goalY, map, path);
        break;
    case 2:
        success = qtRosNode->call_dijkstra_search(startX, startY, goalX, goalY, map, path);
        break;
    case 3:
        success = qtRosNode->call_a_star_search(startX, startY, goalX, goalY, map, path);
        break;
    default:
