...
#define LASER_DOWNSAMPLING 10 //10
#define SENSOR_NOISE       1.5 //1.5
#define RESAMPLING_NOISE_POSITION 0.2 //0.2
#define RESAMPLING_NOISE_ANGLE    0.1 //0.1
#define MOVEMENT_NOISE_POSITION   0.1 //0.1
#define MOVEMENT_NOISE_ANGLE      0.05 //0.05
#define THRESHOLD_DISTANCE  0.2 //0.2
#define THRESHOLD_ANGLE     0.2 //0.2
...
std::vector<geometry_msgs::Pose> get_initial_distribution(int N, float min_x, float min_y, float max_x, float max_y)
{
    ...
    for(int i=0; i<N; i++)
    {
        ...
        float a = rnd.uniformReal(-M_PI,M_PI); //Va de -pi a pi
                             ...
                             sensor_msgs::LaserScan& scan_info, std::vector<sensor_msgs::LaserScan>& simulated_scans)
{
    ...
    {
        simulated_scans[i] = *occupancy_grid_utils::simulateRangeScan(map,particles[i],scan_info, false); //"false" indicates unknown cells are not obstacles
    }
}
                                ...
                                std::vector<float>& weights, float& max_weight, float& weights_sum)
{
    ...
    {   
        ...
        {
            //remember that each particle has its position [x,y,z,w] and is diferent for each one.
            d += fabs(simulated_scans[i].ranges[j] - real_scan.ranges[j*LASER_DOWNSAMPLING]);
        }

        d /= simulated_scans[i].ranges.size();
        //as long as the sum of the diferences between real_scan and simulated_scans are small, the weight would be bigger
...
int random_choice(std::vector<float>& weights, float weights_sum)
{
     ...
     */
    float number = rnd.uniformReal(0,weights_sum);
                                                    ...
                                                    std::vector<float>& weights, float weights_sum)
{
    ...
    for (int i = 0; i < particles.size(); i++)
    {
        //index is the particle with the correct probability
        int index = random_choice(weights, weights_sum);

        resampled_particles[i].position.x = particles[index].position.x + rnd.gaussian(0,RESAMPLING_NOISE_POSITION);
        resampled_particles[i].position.y = particles[index].position.y + rnd.gaussian(0,RESAMPLING_NOISE_POSITION);
        
        float angle = (atan2(particles[index].orientation.z, particles[index].orientation.w) * 2) + rnd.gaussian(0,RESAMPLING_NOISE_ANGLE);
...
void move_particles(std::vector<geometry_msgs::Pose>& particles, float delta_x, float delta_y, float delta_t)
{
    ...
    for (int i = 0; i < particles.size(); i++)
    {
        float angle_i = atan2(particles[i].orientation.z, particles[i].orientation.w) * 2;
    
        float x_i = delta_x * cos(angle_i) - delta_y * sin(angle_i) + rnd.uniformReal(0,MOVEMENT_NOISE_POSITION);

        float y_i = delta_x * sin(angle_i) + delta_y * cos(angle_i) + rnd.uniformReal(0,MOVEMENT_NOISE_POSITION);
        
        angle_i += delta_t + rnd.uniformReal(0,MOVEMENT_NOISE_ANGLE);

        particles[i].position.x += x_i;
        particles[i].position.y += y_i;
        particles[i].orientation.w = cos(angle_i/2);
        particles[i].orientation.z = sin(angle_i/2);
    }
...
int main(int argc, char** argv)
{
    ...
    while(ros::ok())
    {
           ...
           fabs(robot_a - last_robot_a) > THRESHOLD_ANGLE)
        {
             ...
             */
            float delta_x_new = delta_x * cos(robot_a) + delta_y * sin(robot_a);
            float delta_y_new = -delta_x * sin(robot_a) + delta_y * cos(robot_a);
            
            move_particles(particles,delta_x_new, delta_y_new, delta_a); //returns particles with the new positions
            simulate_particle_scans(particles, map, scan_info, simulated_scans); //returns the sumulated scans
            calculate_particle_weights(simulated_scans, real_scan, weights, max_weight, weights_sum); //returns waits, wahits_sum, max_weight
