...
def inflate_map(map):
    if rospy.has_param("/navigation/path_planning/inflation_radius"):
        radius = float(rospy.get_param("/navigation/path_planning/inflation_radius")) #con esto podemos cambiar el valor de la inflacio desde la interfaz grafica
    else:
        radius = 1.0 #si no cambiamos el valor de la inflacion desde la interfaz grafica, queremos que este sea del radio del robot, es decir 1 metro.
...
def get_nearness(map):
    if rospy.has_param("/navigation/path_planning/nearness_radius"):
        ...
        radius = 1.0
    print "Calculating nearness for a radius of " + str(radius) + " meters"
