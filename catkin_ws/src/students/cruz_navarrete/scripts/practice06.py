...
MIN_POINTS_PER_LINE = 20 #20
MAX_DIST_P_TO_LINE  = 0.15 #0.15
...
def find_farthests_point(line_equation, points):
    ...
    [A,B,C] = line_equation
    #max_distance is initialized in 0
    max_distance = 0
    farthest_point_index = 0

    for i in range(0, len(points) - 1):
        #points -- is a list of lists, each element is a list [x,y] 
        [x,y] = points[i]
        #This is the distance between each point, to the line given 
        d = abs(A*x + B*y + C)
...
def cluster_by_distance(point_cloud):
    ...
    # return clusters
    #
    clusters = [] #this is clusters list, each element is a set of points which are close each other.
    #point_cloud -- is a list which contains pints lists [x,y]
    #              [x,y]
    cluster = [point_cloud[0]] #this is a points list 

    for i in range(1, len(point_cloud) - 1):

        [xi,yi] = point_cloud[i] #current point
        [xp,yp] = point_cloud[i-1] #previous point
...
def split_clusters(clusters):
    ...
    for i in range(0, len(clusters)):
        ...
        if dist > MAX_DIST_P_TO_LINE:
            line_clusters.append(clusters[i][0:idx])
            line_clusters.append(clusters[i][idx:len(clusters[i])])
        else:
            line_clusters.append(clusters[i])

    return line_clusters #this list has a set of clusters, each cluster forms a line 
...
def extract_lines(point_cloud):
    ...
    # Get a set of lines where each line is given by the first and last points of the correponding cluster:
    ...
    '''
    #line_clusters -- #this is a clusters list, as the next list shown:
