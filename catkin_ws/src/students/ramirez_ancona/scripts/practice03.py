...
def get_smooth_path(original_path, alpha, beta):
    ...
    while gradient_mag > tolerance:
        ...
        for i in range(1, len(original_path)-1):
            ...
            #Calculate the X-value of the i-th component of the gradient (see lecture notes)
            #Calcualte the Y-value of the i-th component of the gradient (see lecture notes)
	    grad_x = alpha*(2*xn_i-xn_ip-xn_in)
	    grad_x +=beta*(xn_i-xo_i)
	    grad_y = alpha*(2*yn_i-yn_ip-yn_in)
	    grad_y +=beta*(yn_i-yo_i)
