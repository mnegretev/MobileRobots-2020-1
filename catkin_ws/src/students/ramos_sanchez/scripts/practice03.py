...
def callback_smooth_path(req):
    ...
    while gradient_mag > tol:
        grad_0 = alpha * (smooth_path.poses[0].pose.position.x - smooth_path.poses[1].position.x)
        grad_0 += beta * (smooth_path.poses[0].pose.position.x - old_path.poses[0].position.x)
