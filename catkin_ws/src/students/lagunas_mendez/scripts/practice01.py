...
def callback_bfs(req):
    ...
    open_list = deque()
    is_in_closed_list = [False]*(req.map.info.width*req.map.info.height)
    is_in_open_list = [False]*(req.map.info.width*req.map.info.height)
    n_values = [sys.maxint]*(req.map.info.width*req.map.info.height)
    previous = [-1]*(req.map.info.width*req.map.info.height)
    ...
    start_idx = int((start_y - req.map.info.origin.position.y)/req.map.info.resolution)*req.map.info.width
    start_idx += int((start_x - req.map.info.origin.position.x)/req.map.info.resolution)
    goal_idx = int((goal_y - req.map.info.origin.position.y)/req.map.info.resolution)*req.map.info.width
