...
def callback_follow_path(path):
    ...
    for i in range(len(path.poses)):
        robot_x, robot_y, robot_a = get_robot_pose(listener)
        d = math.sqrt(pow(robot_x-path.poses[i].pose.position.x,2)+pow(robot_y-path.poses[i].pose.position.y,2))
        while d>0.1:
            print(d)
            cmd_vel = calculate_control(robot_x, robot_y, robot_a,path.poses[i].pose.position.x, path.poses[i].pose.position.y)
            pub_cmd_vel.publish(cmd_vel)
            loop.sleep()
            robot_x, robot_y, robot_a = get_robot_pose(listener)
            d = math.sqrt(pow(robot_x-path.poses[i].pose.position.x,2)+pow(robot_y-path.poses[i].pose.position.y,2))
