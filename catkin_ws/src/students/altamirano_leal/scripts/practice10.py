...
def evaluate(perceptron, image):
    ...
    #
    A = numpy.dot(perceptron,image)
    output = 1 / (1+math.exp(-A))
    return output
...
def train_perceptron(perceptron, dataset, labels, desired_digit):
    ...
    for i in range(len(dataset)):
        lista.append(numpy.asarray(dataset[i]+[-1]))

    perceptron = numpy.asarray(perceptron)
    while numpy.linalg.norm(grad) > tol and intentos < 250:
        grad = numpy.zeros(785)
        for j in range(len(dataset)):
            yA = evaluate(perceptron, lista[-1])
            yB = 1 if labels[j] == desired_digit else 0
            grad = numpy.add((y_hat-y_des)*(y_hat-y_hat*y_hat) * lista[j], grad)
            perceptron = numpy.add(-alpha*grad, perceptron)
        print str(numpy.linalg.norm(grad)) + " Intento: " + str(intentos)
        intentos = intentos + 1
    return perceptronn
