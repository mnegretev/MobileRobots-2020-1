...
def attraction_force(robot_x, robot_y, goal_x, goal_y):
    ...
    alfa = .8
    #fuerza en x 
    mod = ((robot_x-goal_x)*(robot_x-goal_x))+((robot_y-goal_y)*(robot_y-goal_y))
    mod = math.sqrt(mod) 

    dis_x=(robot_x-goal_x)/(mod)  if mod > 0 else  0
    force_x = alfa*(dis_x) 
    #fuerza en y  
    dis_y=(robot_y-goal_y)/(mod)  if mod > 0 else  0
    force_y = alfa*(dis_y)    
...
def rejection_force(robot_x, robot_y, robot_a, laser_readings):
    ...
    for [distance,angle] in laser_readings:
        ...
        if distance < d0  and distance > 0: 
            mag = math.sqrt((1/distance)-(1/d0))
            mag = mag*beta
        else:
            mag = 0

        force_x += math.cos(robot_a + angle)*mag
        force_y += math.sin(robot_a + angle)*mag

    total = len(laser_readings)

    force_y = force_y/total
    force_x = force_x/total 
