    ...
    while len(open_list) > 0 and current != goal_idx:
        ...
        for n in neighbors:
            ...
            if not in_open_list[n]:
                in_open_list[n] = True
                open_list.append(n)
            distances[n]= distances[current] + (goal_idx - ...)
            heuristics[n]= .... #Función Heuristica
            funcion[n]= distances[n] + heuristics[n] 
            if (distances[current] + ) < distances[n]:
                distances[n]= distances[start_idx] + ...
                parent_nodes[n] = current
