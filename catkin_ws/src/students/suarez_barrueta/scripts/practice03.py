...
def callback_smooth_path(req):
    ...
    old_path = req.path
    ##x = old_path.poses[0].pose.position.x ##de esta manera accedo a todas las posiciones 
    smooth_path = Path()
    ...
    while gradient_mag > tol:
        ...
        grad_0 += alpha*(smooth_path.poses[0].pose.position.x - old_path.poses[0].pose.position.x)
        new_x_0 -= delta*grad_0
        for i n range(1, len(old_))
            grad
##y hacemos  lo mismo de x para  y
